...
export default function HeartButton({ isActive, onClick }: HeartButtonProps) {
  const srText = isActive ? "Remove as favorite" : "Mark as favorite";
  const svgStyles = isActive
    ? "fill-red-500 stroke-red-500 transition-all duration-200 ease-linear heart"
    : "fill-transparent stroke-red-500 transition-all duration-200 ease-linear heart";
